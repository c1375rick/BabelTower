...
- HUD 顶栏翻译:游戏清理顶栏行后自动接管显示译文(浮层 5 秒后隐藏)
...
- 名称保护:动态从桥同步 285 条英雄/物品名,翻译时保护不被意译
- 译文缓存:聊天滚动回收后自动重建译文
- 聊天日志:按比赛 ID 落盘,单文件 5MB 自动轮转,30 天自动清理
- 自适应词典:空表起步,高频短语自动学习固化(上限 5000/语言,按频率淘汰)
- Bing 指数退避:限流时自动退避重试,不再立即重打
